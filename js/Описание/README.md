...
## Подробно про `style.css`

Файл: `style.css`

Цель стилей: сделать интерфейс аккуратным и минималистичным.
...
- `margin: 0; padding: 0;` — убираем стандартные отступы браузера.
...
- `max-width: 400px;` — ограничиваем ширину, чтобы было компактно.
- `margin: 40px auto;` — центрируем блок по горизонтали (`auto`) и делаем отступ сверху.
- `background-color: #ffffff;` — белый фон блока.
- `padding: 20px;` — внутренние отступы.
- `border-radius: 6px;` — слегка скругляем углы.
- `box-shadow: ...` — лёгкая тень, чтобы блок отделялся от фона.
...
- делаем отступ снизу, чтобы отделить заголовок от формы.
- выравниваем по центру.

### Блок добавления задачи (инпут + кнопка)
...
- `display: flex;` — чтобы инпут и кнопка стояли в одну линию.
- `gap: 8px;` — расстояние между ними.
- `margin-bottom: 16px;` — отступ вниз перед списком задач.
...
- `flex: 1;` — инпут занимает всю доступную ширину.
- рамка и лёгкое скругление.
...
- простой “синий” стиль.
- `cursor: pointer;` — чтобы было понятно, что это кнопка.
- `:hover` — кнопка темнеет при наведении.
...
- убираем маркеры списка и отступы, чтобы внешний вид контролировался вручную.
...
- `display: flex;` — текст слева, кнопки справа.
- `justify-content: space-between;` — раздвигает элементы по краям.
- `border-bottom` — разделительная линия между задачами.

### Текст задачи и выполненная задача
...
- `.task-text` занимает место, чтобы кнопки не сжимали текст.
- `.done` добавляет зачёркивание и серый цвет.
...
- `.task-buttons` — контейнер для двух кнопок “Готово” и “Удалить”.
- общие стили для кнопок, а цвета задаются отдельными классами:
  - `.done-button` — зелёная,
  - `.delete-button` — красная.
